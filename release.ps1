...
# Bootsharp.FileSystem source location - configurable via env var or parameter
$fsCs = $env:BOOTSHARP_CS_PATH
if (-not $fsCs) { $fsCs = "D:\extra\bootsharp\cs" }
if (-not (Test-Path $fsCs)) { throw "Bootsharp.FileSystem source not found at '$fsCs'. Set BOOTSHARP_CS_PATH environment variable or edit this script." }
...
Write-Host "==> Bootsharp.FileSystem source: $fsCs" -ForegroundColor Yellow
