...
public static class JamlTools
{
    ...
    public static string JamlValidate([Description("JAML or JSON filter text")] string jaml)
    {
    ...
    )
    {
        ...
        try
        {
            plan = JamlSearchBuilder.CreatePlan(config, 0);
        }
        catch (Exception ex)
        {
            return $"PLAN ERROR: {ex.Message}";
        }
        ...
        { /* hit a maxResults/maxSeconds bound — expected */
        }

        var sb = new StringBuilder();
        sb.AppendLine(
            $"Found {seeds.Count} seed(s) for '{config.Name ?? "filter"}' ({config.Deck} {config.Stake}):"
        );
        ...
        if (seeds.Count == 0)
            sb.AppendLine(
                "No matches inside the bound — raise maxSeconds, or the filter is very rare."
            );
        return sb.ToString();
    }
    ...
    public static string JummyValidate([Description("A single JUMMY line")] string line)
    {
