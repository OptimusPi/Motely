...
public static class JamlTools
{
        ...
        [Description("Worker threads (0 = processor count)")] int threads = 0)
    {
        ...
        // CreateSettings is the public door; CreatePlan().Settings went internal in this tree.
        // No rug-sweeping catch: a bad build throws loud with the real error + stack, and the MCP
        // host surfaces it. One wrong clause fails hard here — it does not return a soft note while
        // the engine still spends your compute.
        IMotelySearchSettings baseSettings = JamlSearchBuilder.CreateSettings(config, 0);
