...
import { z } from "zod";
import { getDiagnostics, getCompletions, getHover, Vocab } from "jaml-lang";
const server = new McpServer({
    name: "jaml-mcp",
    version: "1.0.0",
});
// ── validate ──────────────────────────────────────────────────────────────────
server.tool("jaml_validate", "Validate a JAML filter string. Returns errors and warnings with line/character positions.", { text: z.string().describe("Full JAML filter text to validate.") }, async ({ text }) => {
...
// ── completions ───────────────────────────────────────────────────────────────
server.tool("jaml_complete", "Get JAML completions at a cursor offset. Pass the full text and the byte offset.", {
...
// ── hover ─────────────────────────────────────────────────────────────────────
server.tool("jaml_hover", "Get hover documentation for the token at a cursor offset.", {
    text: z.string().describe("Full JAML filter text."),
    offset: z.number().int().describe("Character offset of the cursor."),
}, async ({ text, offset }) => {
    const info = getHover(text, offset);
    if (!info)
        return { content: [{ type: "text", text: "No hover info at this position." }] };
    return { content: [{ type: "text", text: info.markdown }] };
});
// ── vocab ─────────────────────────────────────────────────────────────────────
server.tool("jaml_vocab", "Dump the full JAML vocabulary: discriminators, clause keys, source keys, enum members. Use this to know what's valid before writing a filter.", {
    ...
    const lines = [];
    if (discriminator) {
        const canon = Vocab.Discriminators.find((d) => d.toLowerCase() === discriminator.toLowerCase());
        if (!canon) {
        ...
        if (valEnum)
            lines.push(`  value enum: ${valEnum} → [${members.join(", ")}]`);
        lines.push(`  clause keys: [${clause.join(", ")}]`);
        if (src.length)
            lines.push(`  source keys: [${src.join(", ")}]`);
    }
    else {
