...
var root = new JsonObject
{
    ...
    ["description"] = "Declarative filter language for Balatro seed searches. A JAML document "
        + "specifies the criteria a seed must satisfy; the Motely engine compiles and runs it.",
    ["version"] = ResolveMotelyVersion(),
    ["type"] = "object",
...
// Single source of truth for the engine version — keeps the schema's `version`
// in lockstep with the Motely build that generated it.
static string ResolveMotelyVersion()
{
    var props = File.ReadAllText("Directory.Packages.props");
    var match = System.Text.RegularExpressions.Regex.Match(
        props, @"<MotelyVersion>([^<]+)</MotelyVersion>");
    if (!match.Success)
        throw new InvalidOperationException(
            "MotelyVersion not found in Directory.Packages.props — run jaml-schema.cs from the repo root.");
