...
NSFW_WORDS = [
    ...
    "NUTS", "SACK", "PACKAGE", "JUNK", "MEMBER", "UNIT", "TOOL", "PIECE", "FUCK", "CUNT", "SHIT", "PISS", "CUM", "SPERM", "SEED", "JUICE", "LIQUID", "FLUID",
    "ORGY", "SWING", "PARTY", "CLUB", "BAR", "LOUNGE", "DANCE", "STRIPPER", "POLE",
    "DOLL", "TOY", "DILDO", "VIBRATOR", "RUBBER", "CONDOM", "LUBE", "OIL", "CREAM",
    "LOTION", "WAX", "CANDLE", "FEATHER", "BONDAGE", "FETISH", "ROLEPLAY", "SCENARIO", "SCRIPT", "PLOT", "STORY",
    "FANTASY", "DREAM", "IMAGINE", "WISH", "DESIRE", "CRAVE", "LONGING", "HUNGER", "THIRST", "NEED", "WANT",
    "JIZZ", "SPUNK", "GOO", "GUSH", "SQUIRT", "SPURT", "DRIP", "LEAK", "SPLASH", "SPRAY", "PUSS", "GOOCH", "PUSSY", "VULVA", "VAGINA", "ASSHOLE", "ANUS", "RECTUM", "MFER", "MOTHER"
]
...
def generate_word_combinations(words: List[str], output_file: str, category: str):
    ...
    for word in words:
        for num in ["69", "42", "666", "777", "888", "999", "123", "321", "1", "O"]:
            if len(word) + len(num) <= 8:
                ...
                all_seeds.add((word + num + "Q" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((num + word + "1" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((word + num + "Z" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((num + word + "Q" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((word + num + "9" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((num + word + "8" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((word + num + "7" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((num + word + "6" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((word + num + "5" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((num + word + "4" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((word + num + "3" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((num + word + "2" * (8 - len(word) - len(num)))[:8])
                all_seeds.add((word + num + "1" * (8 - len(word) - len(num)))[:8])
