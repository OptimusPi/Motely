...
/// <summary>
/// Streaming <see cref="IMotelySeedProvider"/> backed by DuckDB.
/// Handles parquet, CSV, local files, and HTTP URLs — all via <c>DbDataReader</c>.
/// No materialization: rows are pulled one-at-a-time from the reader.
/// </summary>
public sealed class SeedSourceProvider : IMotelySeedProvider, IDisposable
{
    ...
    public SeedSourceProvider(string path)
    {
        ...
        // DuckDB treats backslashes as escape characters in string literals.
        path = path.Replace('\\', '/');

        // threads=1: DuckDB's internal parallelism conflicts with Motely's
        // multi-threaded reader calling NextSeeds on the same native reader.
        ...
        var sql = ext switch
        {
            ...
            _ => $"SELECT column0 AS seed FROM read_csv('{EscapeSql(path)}', header = false)",
        };

        // Count rows with a separate query so progress bars work.
        var countSql = ext switch
        {
            ".parquet" or ".pq" => $"SELECT COUNT(*) FROM read_parquet('{EscapeSql(path)}')",
            _ => $"SELECT COUNT(*) FROM read_csv('{EscapeSql(path)}', header = false)",
        };
