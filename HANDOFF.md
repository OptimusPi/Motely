...
This file (and TASKS.md, HARVEST.md, PLAN.md, JAML_UI_HANDOFF.md) was deleted
in commit `92cc8c2` ("kimi revival", 2026-07-03) as part of a docs/config
cleanup pass. Restored from `92cc8c2^` after the user asked whether anything
of value was lost. It was — the sycophancy callout below, the PROTECTED list,
the cross-repo duplication map. Restoring as a living doc, not a museum piece.

This session's actual work happened mostly in `seedfinder.app` (the MCP-app
consumer), which surfaced real bugs in jaml-ui's published packages:

- **`Any` wildcard never autocompleted.** `packages/jaml-codemirror/src/vocab.ts`
  and `packages/jaml-lsp/src/vocab.ts` built an independent vocab source
  (`MotelyJaml.listItems(kind, "")` per kind) instead of delegating to
  jaml-lang's own `completions.js`, which already unshifts `Any` correctly.
  Fixed in commit `4e88c2f` — verified end-to-end through the real
  `jamlCompletions()` public API against the compiled `dist/index.js`, not
  just source inspection. Scoped narrower than jaml-lang's own behavior on
  purpose: `MotelyJaml.fromYaml` only actually accepts `Any` for
  `joker`/`legendaryJoker` (verified empirically) — `boss: Any`,
  `voucher: Any`, `tag: Any`, `deck: Any` all throw, so jaml-lang's own
  unconditional suggestion of `Any` for every discriminator is itself
  slightly wrong (not fixed here, separate smaller issue).
- **`packages/jaml-codemirror` flipped `private: true → false`** and got a
  real vite bundle build (`dist/index.js`, external `react`/`motely-wasm`)
  in the same session — found as pre-existing uncommitted work, kept as-is
  since the `Any` fix depends on it. First time this package was actually
  publishable.
- **`MotelyJaml­yzer.analyzeSeeds` has a real perf cliff, not yet fixed.**
  ...
  identical wall — `eventRolls` is not an ante-depth limiter despite the
  name. Worked around client-side in seedfinder.app (clamp antes to 8 for
  a throwaway analysis copy of the JAML, never touching the real search
  filter) but the real fix belongs in Motely's C# analyzer.
- **`JamlyzerView`** (exported from jaml-ui's main index) is a complete
  ante-touring component, but caps `maxAnte=8` and generates ante buttons
  `1..n` — **never 0** — and doesn't carry Jimbo styling. Not swapped in
  anywhere; seedfinder.app's hand-rolled X-ray (real sprites, ante-0
  correct) does the job better for that consumer, but this component's
  ante-0 gap is worth knowing if it's reused elsewhere.
- **The old `SeedMascot`** (`src/ui/mascot/SeedMascot.tsx`, task #7 below,
  flagged for dead Tailwind classes) has a spiritual successor:
  `JammyMascot`, exported from jaml-ui's main index as of 3.0.1, renders a
  real `JamlGameCard` sprite (Blueprint by default) with mood states
  (idle/happy/surprised) and a built-in orbital menu — no Tailwind. Worth
  checking whether task #7 is now moot or whether the OLD `SeedMascot` file
  still exists separately and needs the same rip-out other dead code got.
