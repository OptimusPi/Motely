...
Filters are written in JAML (Jimbo's Ante Markup Language) — a filter language that loads to the typed config the engine runs, with real editor tooling (a VS Code extension and a standalone LSP server) generated straight from the engine so the grammar always matches the code. JAML clauses can be written as one-line natural-language lines ("Blueprint in ante 1") or as structured documents — both are JAML.
