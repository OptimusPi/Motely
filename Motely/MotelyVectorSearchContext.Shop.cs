...
ref partial struct MotelyVectorSearchContext
{
        ...
        bool isCached = false)
    {
        ...
        stream.TotalRate = Vector512.Create((double)ShopJokerRate) + 
                          stream.TarotRate + stream.PlanetRate + 
                          stream.PlayingCardRate + stream.SpectralRate;
    ...
    public MotelyItemVector GetNextShopItem(ref MotelyVectorShopItemStream stream)
    {
        ...
        var isPlanet = Vector512.AndNot(Vector512.AndNot(
            Vector512.LessThan(itemTypePoll, stream.PlanetRate), isJoker), isTarot);
        itemTypePoll -= stream.PlanetRate;
        
        var isPlayingCard = Vector512.AndNot(Vector512.AndNot(Vector512.AndNot(
            Vector512.LessThan(itemTypePoll, stream.PlayingCardRate), isJoker), isTarot), isPlanet);
        itemTypePoll -= stream.PlayingCardRate;
        
        var isSpectral = Vector512.AndNot(Vector512.AndNot(Vector512.AndNot(Vector512.AndNot(
            Vector512.LessThan(itemTypePoll, stream.SpectralRate), isJoker), isTarot), isPlanet), isPlayingCard);
        ...
        var result = Vector256.ConditionalSelect(jokerMask, joker.Value,
                     Vector256.ConditionalSelect(tarotMask, tarot.Value,
                     Vector256.ConditionalSelect(planetMask, planet.Value,
                     Vector256.ConditionalSelect(spectralMask, spectral.Value,
