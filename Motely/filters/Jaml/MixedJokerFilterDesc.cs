    ...
    : IMotelySeedFilterDesc<MixedJokerFilterDesc.MixedJokerFilter>
{
    ...
    public MixedJokerFilter CreateFilter(ref MotelyFilterCreationContext ctx)
    {
        ...
        for (int i = 0; i < _clause.Jokers.Length; i++)
        {
            ...
            {
                throw new InvalidOperationException($"Mixed joker {_clause.Jokers[i]} not found in MotelyItemType");
            }
        }
        ...
        return new MixedJokerFilter(_clause, targetTypes, shopIndices.ToArray(), boosterIndices.ToArray(), maxShopItem, maxBoosterPack);
    }
