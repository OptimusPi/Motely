    ...
    : IMotelySeedFilterDesc<StartingDrawFilterDesc.StartingDrawFilter>
{
    ...
    public struct StartingDrawFilter(StartingDrawClause clause) : IMotelySeedFilter
    {
        ...
        public VectorMask Filter(ref MotelyVectorSearchContext ctx)
        {
            ...
            return ctx.SearchIndividualSeeds((ref MotelySingleSearchContext ctx) =>
            {
                int matchCount = 0;
                ...
                foreach (int ante in clause.Antes)
                {
                    ...
                    for (int i = 0; i < handSize; i++)
                    {
                        ...
                        bool rankMatch = !clause.Rank.HasValue || card.PlayingCardRank == clause.Rank.Value;
                        bool suitMatch = !clause.Suit.HasValue || card.PlayingCardSuit == clause.Suit.Value;
