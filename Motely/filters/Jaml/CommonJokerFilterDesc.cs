    ...
    : IMotelySeedFilterDesc<CommonJokerFilterDesc.CommonJokerFilter>
{
    ...
    public CommonJokerFilter CreateFilter(ref MotelyFilterCreationContext ctx)
    {
        ...
        for (int i = 0; i < _clause.Jokers.Length; i++)
        {
            ...
            {
                throw new InvalidOperationException($"Common joker {_clause.Jokers[i]} not found in MotelyItemType");
            }
        }
        ...
        return new CommonJokerFilter(_clause, targetTypes, shopIndices.ToArray(), boosterIndices.ToArray(), maxShopItem, maxBoosterPack);
    }
