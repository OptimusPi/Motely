    ...
    : IMotelySeedFilterDesc<CavendishExtinctFilterDesc.CavendishExtinctFilter>
{
    ...
    {
        Debug.Assert(_clause.Rolls.Length > 0, "Cavendish clause must provide at least one roll index.");
        int[] sortedRolls = [.. _clause.Rolls];
        Array.Sort(sortedRolls);
        return new CavendishExtinctFilter(sortedRolls, _clause.Min, _clause.Luck);
    }

    public struct CavendishExtinctFilter(int[] sortedRolls, int min, double luck) : IMotelySeedFilter
    {
