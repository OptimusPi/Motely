...
|----------------|-----------------------------------|
| Diagnostics, completion, hover for `.jaml` / `.jummy` | Seed search (VS Code extension + `motely-wasm-compat`) |
| Schema-driven enum/value hints (reads `jaml.schema.json`) | YAML extension’s structural validation UI (Red Hat YAML) |
