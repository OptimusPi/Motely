...
- **Schema:** Canonical file is repo-root [`jaml.schema.json`](https://github.com/OptimusPi/MotelyJAML/blob/master/jaml.schema.json). `esbuild.mjs` copies it into this folder for Red Hat YAML (`./jaml.schema.json`). The npm package `@motely/jaml-schema` is only for **external** consumers of the schema on npm, not for this extension.
