...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("jaml.newNotebook", async () => {
      const nb = new vscode.NotebookData([
        new vscode.NotebookCellData(vscode.NotebookCellKind.Markup, "# JAML Seed Search\nEdit the filter below and run it with **Ctrl+Shift+Enter** or the ▶ button.", "markdown"),
        new vscode.NotebookCellData(vscode.NotebookCellKind.Code, "name: My Filter\ndeck: Red\nstake: White\nmust:\n  - joker: Blueprint\n    antes: [1, 2]\n", "jaml"),
      ]);
      const doc = await vscode.workspace.openNotebookDocument("jaml-notebook", nb);
