...
| **S8.P3** | Final lock: **≥ 92% line**, **≥ 80% branch**, stryker smoke, coverlet threshold doc | R4 + threshold recipe in this file | **coverage gates met — 92.09% line / 83.96% branch, 857 green. Threshold gate below passed on this run. Stryker (R4): blocked — dotnet-stryker 4.16 re-runs Motely.Generators over its mutated compilation and the MOTJAML001 duplicate-wire guard fires ('joker'/'jokers' → MotelyJoker + rarity enums) even with --mutate scoped to one desc file; log at `%TEMP%/s8p3-stryker.log`. Next verb: make the generator idempotent under Stryker's compilation (or emit-once guard keyed on syntax tree), then rerun scoped. Findings 9–10 logged. MotelyVectorUtils hardware-dead exclude applied with operator go (AdvSimd confirmed live on operator's ARM64 Mac — the merge verb in section D stands)** |
