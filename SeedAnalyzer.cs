...
public struct AnalyzerFilterDesc : IMotelySeedFilterDesc<AnalyzerFilterDesc.AnalyzerFilter>
{
    ...
    public struct AnalyzerFilter : IMotelySeedFilter
    {
        ...
        public bool CheckSeed(ref MotelySingleSearchContext ctx)
        {
            ...
            for (int ante = 1; ante <= 8; ante++)
            {
                ...
                for (int i = 0; i < slots; i++)
                {
                    ...
                    if (item.Type == ShopState.ShopItem.ShopItemType.Empty) continue;
                    
                    string itemStr = FormatShopItem(ref item);
                    Console.WriteLine($"{shopNum}) {itemStr}");
                    shopNum++;
                }
                
                // TODO: Show full shop queue beyond initial slots
                Console.WriteLine();
                ...
                if (pack1 != 0)
                {
                    ShowPackContentsInline(ref ctx, ante, (MotelyBoosterPack)pack1, 1);
                }
                if (pack2 != 0)
                {
                    ShowPackContentsInline(ref ctx, ante, (MotelyBoosterPack)pack2, 2);
                }
        ...
        private static string FormatShopItem(ref ShopState.ShopItem item)
        {
            switch (item.Type)
            {
                case ShopState.ShopItem.ShopItemType.Joker:
                    var jokerName = FormatJokerName(item.Joker.ToString());
                    if (item.Edition != MotelyItemEdition.None)
                        jokerName = $"{item.Edition} {jokerName}";
                    return jokerName;
                case ShopState.ShopItem.ShopItemType.Tarot:
                    return FormatTarotName(item.Tarot.ToString());
                case ShopState.ShopItem.ShopItemType.Planet:
                    return item.Planet.ToString();
                default:
            ...
            try
            {
                switch (pack)
                {
                    case MotelyBoosterPack.Arcana:
                        ...
                        for (int i = 0; i < 3; i++)
                        {
                            var card = ctx.GetNextTarot(ref arcanaStream);
                            arcanaCards.Add(FormatTarotName(((MotelyTarotCard)(card.Value & 0xFF)).ToString()));
                        }
