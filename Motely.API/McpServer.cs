...
public class McpServer
{
    ...
    private async Task<(string jaml, string reasoning, string? error, string? rawJaml, string? cleanedJaml, string? finalJaml, string? validationError)> GenerateJamlOnlyAsyncInternalWithSteps(string userPrompt, string originalPrompt)
    {
        ...
        try
        {
            ...
            var jamlFilter = await GenerateJamlWithAIAsync(userPrompt, originalPrompt: originalPrompt);
            rawJaml = jamlFilter;
            ...
            if (!JamlConfigLoader.TryLoadFromJamlString(jamlFilter, out var validatedConfig, out validationError) || validatedConfig == null)
            {
                ...
                var errorMsg = $"JAML validation failed: {validationError}\n\n=== TRANSFORMATION PIPELINE ===\n" +
                              $"Raw from AI:\n{rawJaml}\n\n" +
                              $"After CleanMarkdown:\n{cleanedJaml}\n\n" +
                              $"After EnsureJamlHeader:\n{finalJaml}\n\n" +
                              $"Validation Error: {validationError}";
                _logger.LogError($"JAML validation failed for prompt: '{originalPrompt}'\nError: {validationError}\nGenerated JAML:\n{jamlFilter}");
                
                return (jamlFilter, string.Empty, errorMsg, rawJaml, cleanedJaml, finalJaml, validationError);
            }
        ...
        catch (Exception ex)
        {
            ...
            return (string.Empty, string.Empty, $"Error: {ex.Message}", rawJaml, cleanedJaml, finalJaml, validationError);
        }
    }
    
    private async Task<(string jaml, string reasoning, string? error)> GenerateJamlOnlyAsyncInternal(string userPrompt, string originalPrompt)
    {
        var (jaml, reasoning, error, _, _, _, _) = await GenerateJamlOnlyAsyncInternalWithSteps(userPrompt, originalPrompt);
    ...
    public async Task<McpResponse> ProcessPromptAsync(string prompt)
    {
        try
        {
            ...
            if (!JamlConfigLoader.TryLoadFromJamlString(jamlFilter, out var validatedConfig, out _) || validatedConfig == null)
            {
    ...
    /// <summary>
    /// Refines user prompt through multi-step AI agent pipeline (backward compatibility)
    /// </summary>
    ...
    /// <summary>
    /// STEP 2: Trim trailing salutations, frustrations, and incomplete thoughts
    /// PUBLIC for notebook access
    /// </summary>
    ...
    /// <summary>
    /// STEP 3: Sensibility check - remove incomplete thoughts that don't match any valid items
    /// LESS AGGRESSIVE: Only remove obvious garbage, keep everything else
    /// PUBLIC for notebook access
    /// </summary>
    ...
    /// <summary>
    /// Uses LLM to refine/clean the user prompt (RAG approach)
    /// Extracts core search intent, fixes typos, removes filler words
    /// </summary>
    private async Task<string> RefinePromptWithLLMAsync(string rawPrompt)
    {
        ...
        try
        {
            ...
            var refinementPrompt = $@"Extract the core search intent from this user prompt. Remove filler words, fix obvious typos, but keep ALL meaningful search terms and requirements.

User prompt: ""{rawPrompt}""

Return ONLY the cleaned, refined prompt with no explanations or markdown:";

            var response = await _httpClient.PostAsJsonAsync(_workerUrl, new
            {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                try
                {
                    using var stream = new System.IO.MemoryStream(System.Text.Encoding.UTF8.GetBytes(rawContent));
                    var json = await System.Text.Json.JsonSerializer.DeserializeAsync<JsonElement>(stream);
                    
                    if (json.TryGetProperty("jaml", out var jaml) && jaml.ValueKind == JsonValueKind.String)
                    {
                ...
                catch
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(refined))
                    {
                        _logger.LogDebug($"LLM refined prompt (plain text): '{rawPrompt}' → '{refined}'");
                        return refined;
                    }
                }
            }
    ...
    /// </summary>
    private async Task<string> GenerateJamlWithAIAsync(string userPrompt, string? originalPrompt = null)
    {
        if (string.IsNullOrEmpty(_workerUrl))
        {
            throw new InvalidOperationException("Cloudflare Worker URL not configured. Please set WorkerUrl in appsettings.json");
        ...
        _logger.LogInformation($"NOTE: Worker should have system prompt hardcoded. Current system prompt length: {GetSystemPrompt().Length} characters");
        ...
        try
        {
            ...
            try
            {
                ...
                using var stream = new System.IO.MemoryStream(System.Text.Encoding.UTF8.GetBytes(rawResponseContent));
                workerResponse = await System.Text.Json.JsonSerializer.DeserializeAsync<JsonElement>(stream);
                
                if (workerResponse.ValueKind == JsonValueKind.Object)
                {
                    // Check for error response first
                    if (workerResponse.TryGetProperty("success", out var successProp) && 
                        successProp.ValueKind == JsonValueKind.False)
                    {
                        ...
                        throw new HttpRequestException($"Worker error: InternalServerError - {rawResponseContent}");
                    }
                    ...
                    else if (workerResponse.TryGetProperty("config", out var configProperty))
                    {
                        ...
                        try
                        {
                            ...
                            if (config != null)
                            {
                                ...
                                // STEP 4: Ensure completeness - add defaults if needed
                                RefineStep4_EnsureCompleteness(config, originalPrompt ?? userPrompt);
                            ...
                            {
                                throw new InvalidOperationException("Failed to deserialize config from worker response");
                            }
                        }
                        catch (Exception configEx)
                        {
                            ...
                            _feedbackService?.LogFailure(
                                ...
                                context: new { configJson = configJson.Length > 1000 ? configJson.Substring(0, 1000) + "..." : configJson }
                            );
                            
                            // Include the raw config JSON in error for debugging
                            var errorPreview = configJson.Length > 500 ? configJson.Substring(0, 500) + "..." : configJson;
    ...
    private string CleanMarkdown(string text)
    {
        ...
        if (text.TrimStart().StartsWith("{") && text.Contains("\"jaml\""))
        {
            try
            {
                ...
                if (parsed.TryGetProperty("jaml", out var jamlProp) && jamlProp.ValueKind == JsonValueKind.String)
                {
        ...
        var yamlMatch = System.Text.RegularExpressions.Regex.Match(text, @"(?:^|\n)(name:|deck:|must:|should:|mustNot:)", System.Text.RegularExpressions.RegexOptions.Multiline);
            ...
            if (string.IsNullOrWhiteSpace(lines[i]) && i > 0)
            {
                ...
                if (i + 1 < lines.Length)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(nextLine) && 
                        !System.Text.RegularExpressions.Regex.IsMatch(nextLine, @"^[\w-]+:|^[\s]*-[\s]*(joker|voucher|tarot|planet|spectral|soulJoker|tag|boss|playingCard|event|and|or):", System.Text.RegularExpressions.RegexOptions.IgnoreCase))
                    {
                        yamlEnd = i;
                        break;
                    }
    ...
    /// <summary>
    /// System prompt to guide AI and handle voice input typos
    /// Based on legacy genie pattern: schema-aware, few-shot examples, slang handling
    /// </summary>
    public string GetSystemPrompt()  // Made public so it can be accessed by admin endpoint
    {
...
{failureContext}";
    ...
    /// <summary>
    /// Load complete item catalog from JSON file and format for AI prompt
    /// This gives the AI a complete reference of all items with their types
    /// </summary>
    private string GetCompleteItemCatalog()
    {
        try
        {
            ...
            if (catalog.TryGetProperty("jokers", out var jokers))
            {
                ...
                {
                    var commonList = common.EnumerateArray().Select(j => j.GetString()).Where(s => !string.IsNullOrEmpty(s));
                    catalogText.AppendLine($"  Common ({commonList.Count()}): {string.Join(", ", commonList)}");
                }
                if (jokers.TryGetProperty("uncommon", out var uncommon))
                {
                    var uncommonList = uncommon.EnumerateArray().Select(j => j.GetString()).Where(s => !string.IsNullOrEmpty(s));
                    catalogText.AppendLine($"  Uncommon ({uncommonList.Count()}): {string.Join(", ", uncommonList)}");
                }
                if (jokers.TryGetProperty("rare", out var rare))
                {
                    var rareList = rare.EnumerateArray().Select(j => j.GetString()).Where(s => !string.IsNullOrEmpty(s));
                    catalogText.AppendLine($"  Rare ({rareList.Count()}): {string.Join(", ", rareList)}");
                }
                if (jokers.TryGetProperty("legendary", out var legendary))
                {
                    var legendaryList = legendary.EnumerateArray().Select(j => j.GetString()).Where(s => !string.IsNullOrEmpty(s));
                    catalogText.AppendLine($"  Legendary ({legendaryList.Count()}): {string.Join(", ", legendaryList)}");
                }
            }
            catalogText.AppendLine();
            ...
            {
                var voucherList = vouchers.EnumerateArray().Select(v => v.GetString()).Where(s => !string.IsNullOrEmpty(s));
            ...
            {
                var tarotList = tarotCards.EnumerateArray().Select(t => t.GetString()).Where(s => !string.IsNullOrEmpty(s));
            ...
            {
                var planetList = planetCards.EnumerateArray().Select(p => p.GetString()).Where(s => !string.IsNullOrEmpty(s));
            ...
            {
                var spectralList = spectralCards.EnumerateArray().Select(s => s.GetString()).Where(s => !string.IsNullOrEmpty(s));
            ...
            catalogText.AppendLine("  - When user says \"hanging chad\" or \"hangingchad\", use type: \"Joker\", value: \"HangingChad\"");
            catalogText.AppendLine("  - Always check the catalog above to determine the correct type!");
            catalogText.AppendLine("  - Use fuzzy matching: ignore spaces, hyphens, case differences");
    ...
    /// <summary>
    /// Fallback hardcoded catalog if JSON file is missing
    /// </summary>
...
CRITICAL REMINDERS:
  ...
  - Use fuzzy matching: ignore spaces, hyphens, case differences";
    ...
    /// <summary>
    /// Generate simple joker name mapping: ""Display Name"" → EnumName
    /// Simple approach: split PascalCase enum names back into display names
    /// </summary>
    private string GetJokerNameMapping()
    {
        ...
        var commonJokers = new[] {
            ...
            "HalfJoker", "CreditCard", "Banner", "MysticSummit", "EightBall",
            "Misprint", "RaisedFist", "ChaostheClown", "ScaryFace", "AbstractJoker",
            "DelayedGratification", "GrosMichel", "EvenSteven", "OddTodd", "Scholar",
            "BusinessCard", "Supernova", "RideTheBus", "Egg", "Runner",
            "IceCream", "Splash", "BlueJoker", "FacelessJoker", "GreenJoker",
            "Superposition", "ToDoList", "Cavendish", "RedCard", "SquareJoker",
            "RiffRaff", "Photograph", "ReservedParking", "MailInRebate", "Hallucination",
            "FortuneTeller", "Juggler", "Drunkard", "GoldenJoker", "Popcorn",
            "WalkieTalkie", "SmileyFace", "GoldenTicket", "Swashbuckler", "HangingChad",
            "ShootTheMoon"
        };
        
        // Uncommon jokers
        var uncommonJokers = new[] {
            "JokerStencil", "FourFingers", "Mime", "CeremonialDagger", "MarbleJoker",
            "LoyaltyCard", "Dusk", "Fibonacci", "SteelJoker", "Hack",
            "Pareidolia", "SpaceJoker", "Burglar", "Blackboard", "SixthSense",
            "Constellation", "Hiker", "CardSharp", "Madness", "Seance",
            "Vampire", "Shortcut", "Hologram", "Cloud9", "Rocket",
            "MidasMask", "Luchador", "GiftCard", "TurtleBean", "Erosion",
            "ToTheMoon", "StoneJoker", "LuckyCat", "Bull", "DietCola",
            "TradingCard", "FlashCard", "SpareTrousers", "Ramen", "Seltzer",
            "Castle", "MrBones", "Acrobat", "SockAndBuskin", "Troubadour",
            "Certificate", "SmearedJoker", "Throwback", "RoughGem", "Bloodstone",
            "Arrowhead", "OnyxAgate", "GlassJoker", "Showman", "FlowerPot",
            "MerryAndy", "OopsAll6s", "TheIdol", "SeeingDouble", "Matador",
            "Satellite", "Cartomancer", "Astronomer", "Bootstraps"
        };
        
        // Rare jokers
        var rareJokers = new[] {
            "DNA", "Vagabond", "Baron", "Obelisk", "BaseballCard",
            "AncientJoker", "Campfire", "Blueprint", "WeeJoker", "HitTheRoad",
            "TheDuo", "TheTrio", "TheFamily", "TheOrder", "TheTribe",
            "Stuntman", "InvisibleJoker", "Brainstorm", "DriversLicense", "BurntJoker"
        };
        ...
        foreach (var joker in commonJokers.Concat(uncommonJokers).Concat(rareJokers).Concat(legendaryJokers))
        {
...
";
    ...
    /// <summary>
    /// Generate a smart filter name from user prompt
    /// </summary>
    private string GenerateFilterNameFromPrompt(string? prompt)
    {
        ...
        clean = System.Text.RegularExpressions.Regex.Replace(clean, @"^(find|search|get|show|give me|i want|i need)\s+", "", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        clean = System.Text.RegularExpressions.Regex.Replace(clean, @"\s+(please|pls|thx|thanks)$", "", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
    ...
    /// <summary>
    /// STEP 4: Ensure completeness - add default Should clause if missing to ensure scoring columns exist
    /// </summary>
    private void RefineStep4_EnsureCompleteness(MotelyJsonConfig config, string originalPrompt)
    {
        ...
        {
            _logger.LogDebug("Adding default Should clause (Egg joker) to ensure scoring columns exist");
    ...
    /// <summary>
    /// Normalize card names to match enum values (e.g., "Lucky Cat" → "LuckyCat", "Oops All Six" → "OopsAll6s")
    /// </summary>
    ...
    /// <summary>
    /// Fix invalid edition values in config - removes invalid editions that AI might generate
    /// </summary>
...
/// <summary>
/// Intermediate refinement steps for debugging/visualization
/// </summary>
