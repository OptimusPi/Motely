...
function app() {
  return {
    ...
    async loadActive() {
      // No /searches/active endpoint - searches are managed via SignalR
      // Just check if we have a currentSearchId from localStorage
      const saved = localStorage.getItem('currentSearchId');
      if (saved) {
        this.currentSearchId = saved;
    ...
    async loadStatus() {
      ...
      try {
        const res = await fetch(`/search/${encodeURIComponent(this.currentSearchId)}`);
        if (res.ok) {
          const data = await res.json();
      ...
      try {
        ...
        const res = await fetch('/search', {
          ...
          body: JSON.stringify({
            ...
            startBatch: this.startBatch || undefined,
            cutoff: this.cutoff || undefined
          })
        });
        if (!res.ok) {
    ...
    async stopSearch() {
      if (!this.currentSearchId) return;
      try {
        const res = await fetch(`/search/${encodeURIComponent(this.currentSearchId)}/stop`, { method: 'POST' });
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
