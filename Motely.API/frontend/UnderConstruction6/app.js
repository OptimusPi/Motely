...
function renderTable() {
  qs('#resultsBadge').textContent = state.results.length;
...
async function loadActive() {
  try { const res=await fetch(`${apiBase}/searches/active`); if (res.ok) { const data=await res.json(); if (data.searches?.length){ state.currentSearchId=data.searches[0].id; await loadStatus(); }}} catch {}
}
