...
public class SearchManager
{
    ...
    private async Task StopAllSearchesAsync()
    {
        ...
        foreach (var kvp in _activeSearches)
        {
            ...
            // Clean up resources immediately for this search
            search.Appender?.Close();
            search.Connection?.Close();
            
            // Wait for the actual search task to complete
            if (search.Executor != null)
            {
                stopTasks.Add(Task.Run(async () => 
                {
                    try
                    {
                        // Wait for the JsonSearchExecutor to finish after cancellation
                        var timeout = TimeSpan.FromSeconds(5);
                        using var cts = new CancellationTokenSource(timeout);
                        
                        while (!search.CancellationToken.Token.IsCancellationRequested && !cts.Token.IsCancellationRequested)
                        {
                    ...
                    {
                        // Expected when timeout occurs or search is cancelled
                    }
                }));
            }
        }
        
        // Wait for all searches to actually stop
