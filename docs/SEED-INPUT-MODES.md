...
| `SeedSourceProvider` | `Motely.DataLake/SeedSourceProvider.cs` | Streams seeds out of *anything DuckDB can open*: CSV/TXT/Parquet/JSON, a JAML file's `seeds:` block, a `.duckdb`/`.db`/`.sqlite`, a directory, `http(s)://` or `s3://`. Database sources are copied into memory and DETACHed up front so the lake stays writable during the run. `FromLakeRoot(root)` = every seed ever saved under the lake root, all filters, deduped. |
| `SeedLakeSink` | `Motely.DataLake/SeedLakeSink.cs` | The seed lake: `<root>/<filterId>.duckdb`, table `seeds(seed PRIMARY KEY)`, `INSERT OR IGNORE`. Root = `--results-path` → `MOTELY_DATALAKE_PATH` → `Seeds/`. Every find hits disk immediately. |
...
3. **Workers never save locally.** `--local-db` / `PoolWorkerOptions.LocalDbPath` are dead. Either wire `SeedLakeSink` (plain `.duckdb` per filter — matches CLI) or the DuckLake extension (DuckDB's sanctioned answer for many writers; see `Seeds/motely.ducklake`), and delete the flag if neither.
