...
public static class MotelyApiFactory
{
    ...
    public static WebApplication CreateApi(string[]? args = null)
    {
        ...
        if (File.Exists(appsettingsPath))
        {
            ...
            Console.WriteLine($"[Init] Loaded appsettings.json from: {appsettingsPath}");
        }
        else
        {
            ...
            if (File.Exists(contentRootPath))
            {
                ...
                Console.WriteLine($"[Init] Loaded appsettings.json from: {contentRootPath}");
            }
            else
            {
                Console.WriteLine($"[Init] WARNING: appsettings.json not found at {appsettingsPath} or {contentRootPath}");
            }
        }
        ...
        // Register GenieFeedbackService and McpServer
        ...
        builder.Services.AddScoped<McpServer>(sp =>
        {
            try
            {
                ...
                {
                    Console.WriteLine($"[Init] Cloudflare Worker URL configured: {workerUrl}");
                }
                else
                {
                    Console.WriteLine("[Init] WARNING: Cloudflare Worker URL not found in configuration");
                }
        ...
        // MCP endpoint for JamlGenie - natural language to JAML translation
