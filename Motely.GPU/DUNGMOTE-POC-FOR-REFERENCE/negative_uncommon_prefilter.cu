/**
 * @file negative_uncommon_prefilter.cu
 * @brief Negative edition uncommon joker pre-filter
 *
 * This tests the idea of using negative edition uncommon jokers from shop slots
...
// Pre-filter check: Count negative uncommon jokers in shop slots for an ante
__device__ int check_negative_uncommon_prefilter(
    const char *seed_str,
    double seed_hash,
    int ante,
    int checks)
{
    ...
    for (int slot = 0; slot < checks; slot++) {
        ...
        // Uncommon: > 0.70 and <= 0.95
        if (rarity_roll > 0.70 && rarity_roll <= 0.95) {
            hit_count++;
        }
    }
...
GPU_KERNEL void uncommon_negative_prefilter_kernel(
    uint64_t start_idx,
    uint64_t num_seeds,
    int *antes_to_check,
    int num_antes,
    int *result_count)
{
    ...
    for (uint64_t i = tid; i < num_seeds; i += stride) {
        ...
        for (int a = 0; a < num_antes; a++) {
            hit_count += check_negative_uncommon_prefilter(seed_str, seed_hash, antes_to_check[a], 10);
        }
...
int main(int argc, char **argv) {
    ...
    uncommon_negative_prefilter_kernel<<<num_blocks, block_size>>>(
