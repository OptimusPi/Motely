# Run script for negative_legendary_prefilter
param(
...
if (-not (Test-Path "negative_legendary_prefilter.exe")) {
...
Write-Host "Running negative_legendary_prefilter..." -ForegroundColor Green
...
if ($OutputFile) {
    ...
    Write-Host "  .\run_test_prefilter.ps1 $Count `"$Antes`" `$lastSeed $OutputFile" -ForegroundColor Gray
    Write-Host ""
}
Write-Host ""

if ($OutputFile) {
    .\negative_legendary_prefilter.exe $Count $Antes $StartSeed $OutputFile
} else {
    .\negative_legendary_prefilter.exe $Count $Antes $StartSeed
}
