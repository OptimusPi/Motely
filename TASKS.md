...
- **#70 SeedMascot** — keep for later; comment out only if it breaks the build, never delete. UPDATE 2026-07-04: `JammyMascot` (main index export, jaml-ui@3.0.1) is the real successor — real `JamlGameCard` sprite, mood states, orbital menu, no Tailwind — wired into seedfinder.app's status row this session. Check whether the OLD `src/ui/mascot/SeedMascot.tsx` (task #7, dead Tailwind) still needs ripping out now that a working replacement exists elsewhere in the library.
