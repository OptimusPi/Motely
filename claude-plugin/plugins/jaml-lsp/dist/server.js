      ...
      getLineOffsets() {
        if (this._lineOffsets === void 0) {
          ...
          if (isLineStart && text.length > 0) {
            lineOffsets2.push(text.length);
          }
          this._lineOffsets = lineOffsets2;
        }
        return this._lineOffsets;
      }
      positionAt(offset) {
        offset = Math.max(Math.min(offset, this._content.length), 0);
        const lineOffsets2 = this.getLineOffsets();
        let low = 0, high = lineOffsets2.length;
        if (high === 0) {
          return Position.create(0, offset);
        }
        while (low < high) {
          const mid = Math.floor((low + high) / 2);
          if (lineOffsets2[mid] > offset) {
            high = mid;
          } else {
            low = mid + 1;
          }
        ...
        const line = low - 1;
        return Position.create(line, offset - lineOffsets2[line]);
      }
      offsetAt(position) {
        const lineOffsets2 = this.getLineOffsets();
        if (position.line >= lineOffsets2.length) {
          return this._content.length;
        } else if (position.line < 0) {
          return 0;
        }
        const lineOffset = lineOffsets2[position.line];
        const nextLineOffset = position.line + 1 < lineOffsets2.length ? lineOffsets2[position.line + 1] : this._content.length;
...
var FullTextDocument2 = class _FullTextDocument {
  ...
  update(changes, version) {
    for (const change of changes) {
      if (_FullTextDocument.isIncremental(change)) {
        ...
        let lineOffsets2 = this._lineOffsets;
        const addedLineOffsets = computeLineOffsets(change.text, false, startOffset);
        if (endLine - startLine === addedLineOffsets.length) {
          for (let i = 0, len = addedLineOffsets.length; i < len; i++) {
            lineOffsets2[i + startLine + 1] = addedLineOffsets[i];
          }
        } else {
          if (addedLineOffsets.length < 1e4) {
            lineOffsets2.splice(startLine + 1, endLine - startLine, ...addedLineOffsets);
          } else {
            this._lineOffsets = lineOffsets2 = lineOffsets2.slice(0, startLine + 1).concat(addedLineOffsets, lineOffsets2.slice(endLine + 1));
          }
        }
        const diff = change.text.length - (endOffset - startOffset);
        if (diff !== 0) {
          for (let i = startLine + 1 + addedLineOffsets.length, len = lineOffsets2.length; i < len; i++) {
            lineOffsets2[i] = lineOffsets2[i] + diff;
          }
        }
  ...
  positionAt(offset) {
    ...
    const line = low - 1;
    offset = this.ensureBeforeEOL(offset, lineOffsets2[line]);
    return { line, character: offset - lineOffsets2[line] };
  }
  offsetAt(position) {
    const lineOffsets2 = this.getLineOffsets();
    if (position.line >= lineOffsets2.length) {
...
// ../jaml-lang/dist/validator.js
var Severity;
(function(Severity2) {
  Severity2[Severity2["Error"] = 1] = "Error";
  Severity2[Severity2["Warning"] = 2] = "Warning";
  Severity2[Severity2["Information"] = 3] = "Information";
  Severity2[Severity2["Hint"] = 4] = "Hint";
})(Severity || (Severity = {}));
...
function offsetOf(lineOffsets2, line, col) {
  return (lineOffsets2[line] ?? 0) + col;
}
...
function validate(text) {
  ...
  function diag(line, col, len, severity, message) {
    const from = offsetOf(offsets, line, col);
    diags.push({ from, to: from + len, severity, message });
  }
  function checkClauseValueEnum(key, rawLine, lineIdx, ind) {
    ...
    const arr = val.match(/^\[(.*)\]$/);
    const tokens = arr ? arr[1].split(",").map((t) => t.trim()).filter((t) => t.length > 0) : [val];
    for (const token of tokens) {
      const norm = token.replace(/\s+/g, "");
      if (norm.toLowerCase() === "any")
        continue;
      if (members.some((m) => m.toLowerCase() === norm.toLowerCase()))
        continue;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ind === 0 && !trimmed.startsWith("-")) {
      ...
      if (key) {
        ...
        if (["must", "should", "mustnot"].includes(key.toLowerCase()))
          inSection = true;
      }
      continue;
    }
        ...
        if (!DISC_SET.has(key.toLowerCase())) {
          ...
          if (val) {
            ...
            if (enumName) {
              const members = Enums[enumName] ?? [];
              const stripped = val.replace(/\s+/g, "");
              if (!stripped.toLowerCase().startsWith("[") && // skip arrays
              val !== "Any" && !members.some((m) => m.toLowerCase() === stripped.toLowerCase())) {
                const col = raw.lastIndexOf(val);
      ...
      if (DISC_SET.has(key.toLowerCase())) {
        ...
        if (val) {
          ...
          if (enumName) {
            const members = Enums[enumName] ?? [];
            const stripped = val.replace(/\s+/g, "");
            if (!stripped.startsWith("[") && val !== "Any" && !members.some((m) => m.toLowerCase() === stripped.toLowerCase())) {
              const col = raw.lastIndexOf(val);
              diag(i, col, val.length, "warning", `Unknown ${enumName} value '${val}'.`);
            }
          }
...
function getContext(text, offset) {
  const lines = text.split("\n");
  let pos = 0;
  let cursorLine = 0;
  let cursorCol = 0;
  ...
  let clauseIndent = -1;
  let discriminatorFound = null;
  let inSourcesBlock = false;
  let sourcesIndent = -1;
  const clauseDiscriminators = /* @__PURE__ */ new Set([
    "boss",
    "joker",
    "jokers",
    "commonJoker",
  ...
  for (let i = cursorLine; i >= 0; i--) {
    ...
    if (isListItem && !inClauseList) {
      ...
      for (let j = i; j <= cursorLine && j < lines.length; j++) {
        ...
        if (j > i && jInd <= clauseIndent)
          break;
        const jContent = stripListMarker(jRaw);
        const keyMatch = jContent.match(/^([\w-]+)\s*:/);
        if (keyMatch) {
          const k = keyMatch[1];
          if (clauseDiscriminators.has(k) || clauseDiscriminators.has(k.charAt(0).toUpperCase() + k.slice(1))) {
  ...
  if (isValuePosition && valueKey) {
    const isDisc = clauseDiscriminators.has(valueKey) || clauseDiscriminators.has(valueKey.charAt(0).toUpperCase() + valueKey.slice(1));
    if (isDisc) {
...
// ../jaml-lang/dist/completions.js
function filterPrefix(items, prefix) {
  if (!prefix)
    return items;
  const lp = prefix.toLowerCase().replace(/\s+/g, "");
  return items.filter((i) => i.label.toLowerCase().startsWith(lp) || i.label.toLowerCase().replace(/\s+/g, "").startsWith(lp));
}
function kwItems(keys, kind = "field") {
  return keys.map((k) => ({ label: k, kind, insertText: `${k}: ` }));
}
function enumItems(enumName, detail) {
  const members = Enums[enumName] ?? [];
  return members.map((m) => ({ label: m, kind: "enum", detail: detail ?? enumName }));
}
var clauseValueEnum = new Map(Object.entries(ClauseKeyValueEnum).map(([k, v]) => [k.toLowerCase(), v]));
function canonDisc(disc) {
  return Discriminators.find((d) => d.toLowerCase() === disc.toLowerCase()) ?? disc;
}
function getCompletions(text, offset) {
  ...
  switch (ctx.kind) {
    ...
    case "root-value": {
      ...
      if (enumName)
        return filterPrefix(enumItems(enumName), ctx.prefix);
      return [];
    }
    case "discriminator":
      return filterPrefix(Discriminators.map((d) => ({ label: d, kind: "keyword", detail: "clause type", insertText: `${d}: ` })), ctx.prefix);
...
// ../jaml-lang/dist/hover.js
var clauseValueEnum2 = new Map(Object.entries(ClauseKeyValueEnum).map(([k, v]) => [k.toLowerCase(), v]));
...
Source keys: ${sourceKeys.map((k) => `\`${k}\``).join(", ")}`;
  ...
  if (ctx.kind === "clause-value" && ctx.valueKey) {
    const enumName = clauseValueEnum2.get(ctx.valueKey.toLowerCase());
    if (enumName) {
      const members = Enums[enumName] ?? [];
...
documents.onDidClose(
  (e) => connection.sendDiagnostics({ uri: e.document.uri, diagnostics: [] })
);
connection.onCompletion((params) => {
  ...
  return getCompletions(doc.getText(), offset).map((item) => ({
    label: item.label,
    kind: item.kind === "keyword" ? import_node.CompletionItemKind.Keyword : item.kind === "enum" ? import_node.CompletionItemKind.EnumMember : item.kind === "field" ? import_node.CompletionItemKind.Field : import_node.CompletionItemKind.Constant,
    detail: item.detail,
...
connection.onHover((params) => {
  ...
  if (!info) return null;
  return { contents: { kind: import_node.MarkupKind.Markdown, value: info.markdown } };
});
