...
Unless the user has specified otherwise, use `tsx` for running TypeScript server files:
...
2. **Missing CSP configuration** - All network requests require CSP; MCP Apps HTML is served as an MCP resource with no same-origin server
