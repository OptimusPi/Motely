...
| Run `npm run build` in `D:\bootsharp\src\js` | **pifreak only** |
| Run `dotnet pack` in `D:\bootsharp\src\cs` or `D:\extra\bootsharp\cs` | **pifreak only** |
| Copy produced `.nupkg` files into `MotelyJAML/.nuget-local/` | agent OK |
| Clear `C:\Users\pifre\.nuget\packages\bootsharp*` global cache | agent OK |
...
```bash
cd src/js && npm run build           # rebuilds dist if instances.ts/imports.ts/etc changed
cd ../cs && bash ./.scripts/pack.sh  # produces Bootsharp{,.Common,.Inject}.<version>.nupkg in .nuget/
```
On Windows without bash, run the equivalent: `dotnet build Bootsharp.Generate -c Release` then `dotnet pack Bootsharp.Common -o .nuget -c Release` (and same for `Bootsharp.Inject`, `Bootsharp`).
...
Triggers that mean the staged `.nupkg` in `.nuget-local/` is stale and the consumer build will fail:
...
When any of those happens: agent reports "Bootsharp `.nupkg` in `.nuget-local/` looks stale because [reason]; pifreak please re-pack per the flow above." Agent does NOT run the upstream build to fix it.

#### Staging produced `.nupkg` files

After pifreak re-packs, agent may:
1. `Copy-Item D:\bootsharp\src\cs\.nuget\Bootsharp*.<version>.nupkg X:\JammySeedFinder\src\MotelyJAML\.nuget-local\`
2. `Copy-Item D:\extra\bootsharp\cs\.nuget\Bootsharp.FileSystem.<version>.nupkg X:\JammySeedFinder\src\MotelyJAML\.nuget-local\`
3. Bump `Bootsharp.FileSystem` version in `Directory.Packages.props` if needed (the time-stamped version changed)
4. `Remove-Item C:\Users\pifre\.nuget\packages\bootsharp,bootsharp.common,bootsharp.inject,bootsharp.filesystem -Recurse -Force` (run `dotnet build-server shutdown` first if any DLL is locked by an MSBuild task host)
5. `dotnet restore Motely.sln` → fresh extraction from the updated `.nupkg`s
