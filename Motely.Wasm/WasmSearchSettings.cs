...
public sealed class WasmSearchSettings
{
    ...
    public WasmSearchSettings WithSequentialSearch()
    {
        try
        {
            return new(_inner.WithSequentialSearch());
        }
        catch (Exception ex)
        {
            Program.ReportWasmError($"[WASM C# EXCEPTION in WithSequentialSearch] {ex}");
            throw;
        }
    }
    ...
    public WasmSearchSettings WithJimmolate()
    {
        try
        {
            return new(_inner.WithAdditionalFilter(new JimmolateFilterDesc(Program.RunJimmolateImport)));
        }
        catch (Exception ex)
        {
            Program.ReportWasmError($"[WASM C# EXCEPTION in WithJimmolate] {ex}");
            throw;
        }
    }

    public WasmSearch CreateSearch() => new(_inner.CreateSearch());

    public WasmSearch Start()
    {
        try
        {
            return new(_inner.Start());
        }
        catch (Exception ex)
        {
            Program.ReportWasmError($"[WASM C# EXCEPTION] {ex}");
            throw;
        }
    }
}
