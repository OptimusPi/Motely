...
async function searchList(yaml, seeds) {
    ...
    const onR = (result) => scoredMatches.push(result);
    ...
    const rows = bareMatches.map((row) => {
        const [seed, score, ...tallies] = String(row).split(",");
        return { seed, score: score === undefined ? null : Number(score), tallies: tallies.map(Number) };
    });
